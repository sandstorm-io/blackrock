...
interface Replica {
  ...
  getLeader @1 () -> (leader :Leader);
  # Gets the leader for this object, starting a new leader if necessary.
...
interface Leader {
  ...
  initObject @0 (key :ObjectKey) -> (factory :Storage.StorageFactory);
  # Create a new object. Only valid to call when the object is not yet created. The
  # returned factory is good for exactly one call, which will create an object with the desired
  # key.

  getObject @1 (key :ObjectKey) -> (cap :OwnedStorage);
...
struct StorageConfig {
  ...
  stragglerTimeoutMs @3 :UInt32 = 100;
  # After a quorum is achieved, wait up to this many additional milliseconds for straggler replicas
  # to respond. If they don't, give up on them.
}
